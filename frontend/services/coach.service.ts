...
export const CoachService = {
  ...
  async explainWithAI(
    ...
    return {
      explanation: [
        `Move ${request.move} was classified as ${request.classification ?? 'analyzed'}.`,
        request.bestMove
          ? `The engine preferred ${request.bestMove}.`
          : '',
        `Evaluation changed from ${
          request.previousValue ?? '—'
        } to ${request.value}.`,
        'Soon this explanation will be generated by the AI coach.',
      ]
        .filter(Boolean)
        .join(' '),
    };
